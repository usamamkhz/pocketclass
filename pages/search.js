			...
			<main className={`flex ${style.wrapper}`}>
				<section className={`w-full pt-14 px-6 ${style.cardContainer}`}>
					...
					<div className="hidden md:inline-flex mb-5 space-x-3 text-gray-800 whitespace-nowrap">
						<p className="px-4 py-2 shadow-lg border rounded-full cursor-pointer hover: shadow-lg active:scale-95 active:bg-gray-100 transition transform duration-100 ease-out">
							Class categories
						</p>
						<p className="px-4 py-2 shadow-lg border rounded-full cursor-pointer hover: shadow-lg active:scale-95 active:bg-gray-100 transition transform duration-100 ease-out">
							Price
				...
				{searchResults.length !== 0 && (
					<section
						className={`hidden xl:inline-flex xl:min-w-[30%] ${style.mapContainer}`}
					>
...
export async function getServerSideProps(context) {
	...
	if (searchInput !== "all") {
		let q = query(collection(db, "classes"), where("Type", "==", searchInput));
